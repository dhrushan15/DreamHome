...
export default function Contacts({onContactRequestsChange }) {
    ...
    useEffect(() => {
        const fetchContactRequests = async () => {
            try {
                ...
                const filteredRequests = data.filter(request => request.sender===userId);
                filteredRequests.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
                setContactRequests(filteredRequests);
                onContactRequestsChange(filteredRequests.length);
            } catch (error) {
